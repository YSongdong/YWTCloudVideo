...
- (void)showWithInfo: (LKBubbleInfo *)info{
    ...
    [UIView animateWithDuration: 0.4 delay:0 usingSpringWithDamping: 0.5 initialSpringVelocity:0.5 options: UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        self->_titleLabel.frame = [info calTitleViewFrameWithTitle:info.title];
        self->_titleLabel.numberOfLines = 0;
        self->_titleLabel.textAlignment = 1;
        ...
        if (info.iconArray == nil || info.iconArray.count == 0) {
            // 显示显示自定义动画
            self->_iconImageView.image = [[UIImage alloc] init];
            self->_currentDrawLayer = [CAShapeLayer layer];
            self->_currentDrawLayer.fillColor = [UIColor clearColor].CGColor;
        ...
        // maskView
        if (self->_currentInfo.isShowMaskView && self->_maskView.hidden) {
            // 本次需要显示，但是之前已经隐藏
